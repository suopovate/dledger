...
public class DLedgerLeaderElector {
    ...
    /**
     * 这个默认为false,那么第一次成为candidate时，可能就会一直都不会进行选举操作，直到这个变成true
     */
    ...
    /**
     * 当角色有变更时，通知处理器，通常的使用场景是 应用层，需要感知 dledger 角色变更
     * 比如 broker 需要知道现在自己是 主节点 还是 从节点，是从节点就不能接受数据的写入。
     */
    ...
    /**
     * 处理leader节点向从节点发送的心跳请求
     */
    public CompletableFuture<HeartBeatResponse> handleHeartBeat(HeartBeatRequest request) throws Exception {

        // 数据安全性校验：主节点id是否属于集群已知节点
        ...
        // 是否请求term已经过期
        // 这段代码算是一个快速判断逻辑，所以没有加锁，其实完全可以直接走下面加锁的逻辑，但是大部分情况是没必要加锁的，所以有这段代码。
        if (request.getTerm() < memberState.currTerm()) {
            return CompletableFuture.completedFuture(new HeartBeatResponse().term(memberState.currTerm()).code(DLedgerResponseCode.EXPIRED_TERM.getCode()));
        } else if (request.getTerm() == memberState.currTerm()) {
            // 假如走到这里的时候，leader不一致了，说明这时候出现了别的leader选举成功，并发了，那就走下面的逻辑，加锁!
            // 注意 由于没加锁，所以可能此时的term也是改变了的。
            // 如果走到这里还相等，就直接给他返回成功，即使在判断完成的下一刻，返回之前，立马换主了，也不管了，让这个旧主，下一个心跳再来。
        ...
        //走到这里说明 产生了并发情况了，或者leader
        //abnormal case
        //hold the lock to get the latest term and leaderId
        synchronized (memberState) {
            // 又判断一次...因为可能在执行到这之前，又收到了其他的心跳，毕竟是并发的。
            if (request.getTerm() < memberState.currTerm()) {
                return CompletableFuture.completedFuture(new HeartBeatResponse().term(memberState.currTerm()).code(DLedgerResponseCode.EXPIRED_TERM.getCode()));
            } else if (request.getTerm() == memberState.currTerm()) {
                // 说明当前应该是candidate，或者刚启动的follower(如果是leader那就是出bug了，同一个term不可能出现两个Leader)
                if (memberState.getLeaderId() == null) {
                    ...
                    //的确不太可能发生，同一个term不可能产生两个leader的，即使网络分区也不可能
                    //就算你有十个节点，分成了两个 5-5 区，也不可能出现这种情况，整个池子的节点数是已知的，没有6个节点同意，不可能成为新leader
                    //如果出现平均网络分区，结果就是整个集群瘫痪，永远选不出主，因为每个节点想成为主，必须有 n/2 + 1个节点的支持。
    ...
    /**
     * @param term 新term
     * @param leaderId 新term对应的Leader节点的id
     */
     ...
     * 处理候选人投票请求，选举的核心逻辑。
     *
     * @param request candidate节点发出的投票请求
     * @param self 是否是当前节点发出的请求
     * @return 投票的结果
     */
    ...
    /**
     * 向其他的节点发送心跳：同步数据 + 维持Leader角色。
     *
     * @param term 当前term
     * @param leaderId 当前节点的id
     * @throws Exception
     */
        ...
        if (DLedgerUtils.elapsed(lastLeaderHeartBeatTime) > 2L * heartBeatTimeIntervalMs) {
            synchronized (memberState) {
                ...
                if (memberState.isFollower() && DLedgerUtils.elapsed(lastLeaderHeartBeatTime) > (long) maxHeartBeatLeak * heartBeatTimeIntervalMs) {
                    ...
                    // 这里竟然不是直接升级term，在论文中应该是先升级term的，估计这里是优化过了。
    ...
    /**
     * 主动维护状态
     * 分为 leader、follower、candidate三种角色的情况。
     *
     * The core method of maintainer.
     * Run the specified logic according to the current role: candidate => propose a vote.
